...
export default async function AdminCashPage() {
  ...
  const current = summaries.find((summary) => summary.session.status === "open");
  const closed = summaries.filter((summary) => summary.session.status === "closed");
    ...
    <div className={styles.page}>
      <header className={styles.header}>
        ...
        <p className={styles.subtitle}>
          Supervisa la jornada compartida y asigna la responsabilidad de apertura y cierre.
        </p>
      </header>

      <section className={`${styles.panel} ${styles.panelStack}`}>
        <h2 className={styles.cardTitle}>Permiso de responsable</h2>
            ...
            <div className={styles.headerRow}>
              <div>
                ...
                <p className={styles.help}>
                  {operator.can_manage_cash_session ? "Responsable autorizada" : "Sin permiso de caja"}
                </p>
              </div>
      ...
      {current ? (
        <section className={`${styles.panel} ${styles.panelStack}`}>
          ...
          <h2 className={styles.cardTitle}>Cierre administrativo</h2>
          <CloseCashSessionForm sessionId={current.session.id} />
        </section>
      ) : (
        <section className={`${styles.panel} ${styles.panelStack}`}>
      ...
      <section className={styles.panelStack}>
        <h2 className={styles.cardTitle}>Jornadas anteriores</h2>
        {closed.length === 0 ? (
