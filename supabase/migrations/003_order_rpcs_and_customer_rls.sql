...
begin
  ...
  select history.order_id, history.changed_by
  into v_existing_order_id, v_existing_changed_by
  from public.order_status_history as history
  where history.operation_id = p_operation_id;

  if v_existing_order_id is not null then
    if v_existing_changed_by = v_actor_id
       and exists (
         select 1
         from public.order_status_history as history
         where history.operation_id = p_operation_id
      ...
      select
        ...
        existing.balance_due,
        true
      from public.orders as existing
      where existing.id = v_existing_order_id;
  ...
  -- discount remains zero in this phase; column kept for future use.
  v_discount := 0;
  v_total := v_subtotal - v_discount;
  v_amount_paid := 0;
  v_balance_due := pg_catalog.greatest(v_total - v_amount_paid, 0);
    ...
    when unique_violation then
      if exists (
        ...
        select
          ...
          true
        from public.orders as existing
        join public.order_status_history as history
          on history.order_id = existing.id
        where history.operation_id = p_operation_id
          and history.changed_by = v_actor_id;
  ...
  begin
    if p_to_status = ''in_process'' then
      ...
      set
        ...
        cancelled_at = null,
        cancel_reason = null,
        delivery_with_balance_authorized_by = case
          when orders.balance_due > 0 then v_actor_id
          else null
        end,
        delivery_with_balance_reason = case
          when orders.balance_due > 0 then v_reason
          else null
        end
      where orders.id = p_order_id;
  ...
  exception
    when unique_violation then
      if exists (
        ...
        from public.order_status_history as history
        where history.operation_id = p_operation_id
          and history.order_id = p_order_id
          and history.to_status = p_to_status
          and history.changed_by = v_actor_id
